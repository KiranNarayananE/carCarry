...
const UserList = () => {
  ...
  const block= async (id,verified) => {
    try {
      const response = await AxiosInstance.patch("/admin/user",{id:id,verified:verified}, {
        headers: { Authorization: `Bearer  ${token}` },
      });
  ...
  if(event.target.value){
   let updateUse= list.filter((item)=>item.name.toLowerCase().indexOf(event.target.value.toLowerCase()) !== -1 )
   if(!updateUse[0]){
  ...
  useEffect(() => {
    const user = async () => {
      try {
        await getUserList(token).then((user) => {
          setList(user);
          setListShow(user)
        });
      } catch (error) {
                            ...
                            <tr className="border-b border-gray-200 hover:bg-gray-100" >
                                ...
                                <td className="py-3 px-6 text-center">
                                    <div className="flex item-center justify-center">
                                        ...
                                        <div className="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" key={user._id} onClick={()=>block(user._id,user.verified)}>
