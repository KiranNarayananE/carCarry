...
const WalletLeft = ({ wallet }) => {
  ...
  const addAmount = async (cash, token) => {
    try {
      const response = await axiosInstance.post("/driver/add-cash", { amount: cash }, { headers: { Authorization: `Bearer ${token}` } });
      return response;
    } catch (error) {
    ...
    <>
      <div className="sm:w-4/12 mb-10 px-4  bg-gray-200 hover:bg-gray-300 rounded-lg">
        <div className="h-auto text-start mt-4 ">
          <h3 className="text-base">AMOUNT TO ADD</h3>
          <h1 className="text-3xl mt-4 text-black font-bold">₹ {value ? value : "00.00"}</h1>
          <p className="text-xl mt-4 text-black font-bold">Add Money To Your Wallet</p>
