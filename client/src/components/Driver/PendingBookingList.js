...
const PendingBookingList = (trip) => {
  ...
  const acceptPendingBookings = async (id, token) => {
    try {
      const response = await AxiosInstance.patch("/driver/accept-booking", { id }, { headers: { Authorization: `Bearer ${token}` } });
      return response;
    } catch (error) {
        ...
        <div className="h-full flex items-center border-gray-800 border p-4 rounded-lg">
          ...
          <div className="flex-grow">
            <div className="flex gap-2 mb-2 items-center text-white">
                {console.log(trip)}
              <h1 className="text-lg  font-bold text-black">{trip.data.location.pickup.split(",")[0]}</h1>
              <ArrowRightAltIcon />
              <h1 className="text-lg  font-bold text-black">{trip.data.location.dropoff.split(",")[0]}</h1>
            </div>
            <div className="flex justify-between items-center">
              <h2 className="text-gray-200">
                Date : <span className="text-red-500 font-semibold text-black">{trip.data.date}</span>
              </h2>
