...
const Wallet = () => {
  ...
  useEffect(() => {
    ...
    const getBalance = async (token) => {
        try {
          const response = await axiosInstance.get("/driver/wallet-balance", { headers: { Authorization: `Bearer ${token}` } });
          return response;
        } catch (error) {
      ...
      const getWallet = async (token) => {
        try {
          const response = await axiosInstance.get("/driver/wallet-details", { headers: { Authorization: `Bearer ${token}` } });
          return response;
        } catch (error) {
          return error.response;
        }
